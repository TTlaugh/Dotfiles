...
#[0]##################################################################################################################################
    if [[ "$chosen" = "0" ]] || [[ -z "$chosen" ]]; then
        ...
        unpackage="$(sudo pacman -Qtdq | wc -l) packages"
...
#[2]##################################################################################################################################
        ...
        [[ "$unusedpackages" = "" ]] && printf "${BOLD}No unused packages to remove. Enter to continue.${RESET}" && read && continue
...
#[3]##################################################################################################################################
        ...
        if [[ "$chosen3" = "1" ]]; then
            ...
            printf "\n${BOLD}Enter to continue.${RESET}" && read && continue
        elif [[ "$chosen3" = "2" ]]; then
